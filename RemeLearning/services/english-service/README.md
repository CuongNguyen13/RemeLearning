...
LOCKED/UNLOCKED/IN_PROGRESS/PASSED gating state machine with a **chain of AI-generated Sections per
topic** — each Section a short passage plus Supertonic-synthesized audio, backed by a **random
10-15-question** multiple-choice pool. The chain length is also random, **5-10 Sections per topic**,
deterministic from `topicId` (`targetSectionCount`, not stored — always the same number for the same
topic). Four endpoints under `ListeningLibraryController` (`/api/v1/learn/listening/library`):
...
- `POST /{userId}/sections/{sectionId}/answers` — scores a submitted answer set for that one Section
  ...
  next topic by `sequenceOrder` once **every** Section in the chain has been passed, not on this one
  Section alone.
