-- Grammar Library: 60 fixed grammar topics organized beginner -> intermediate -> advanced, each
-- with an AI-generated theory page (generated once, reused forever) + a reusable 8-10 question
-- pool, drilled via sessions with a pass/retry/unlock-next-topic progression per learner.

CREATE TABLE grammar_library_topics (
...
INSERT INTO grammar_library_topics (code, name, description, level, sequence_order) VALUES
...
CREATE TABLE grammar_library_contents (
    id BIGSERIAL PRIMARY KEY,
    topic_id BIGINT UNIQUE NOT NULL REFERENCES grammar_library_topics (id),
    explanation_en TEXT NOT NULL,
    explanation_vi TEXT NOT NULL,
    illustration_text TEXT NOT NULL,
    examples_json TEXT NOT NULL,
    generated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE TABLE grammar_library_questions (
    id BIGSERIAL PRIMARY KEY,
    topic_id BIGINT NOT NULL REFERENCES grammar_library_topics (id),
...
CREATE INDEX idx_grammar_library_questions_topic_id ON grammar_library_questions (topic_id);
...
CREATE INDEX idx_grammar_topic_progress_user_id ON grammar_topic_progress (user_id);

CREATE TABLE grammar_library_sessions (
    id BIGSERIAL PRIMARY KEY,
    user_id VARCHAR(100) NOT NULL,
    topic_id BIGINT NOT NULL REFERENCES grammar_library_topics (id),
...
CREATE INDEX idx_grammar_library_sessions_user_id ON grammar_library_sessions (user_id);

CREATE TABLE grammar_library_session_answers (
    id BIGSERIAL PRIMARY KEY,
    session_id BIGINT NOT NULL REFERENCES grammar_library_sessions (id),
...
CREATE INDEX idx_grammar_library_session_answers_session_id ON grammar_library_session_answers (session_id);
