-- Speaking Library: fixed 60-topic taxonomy mirroring grammar_library_topics'/listening_library_topics'
-- topic set (same names/order, independent ids/table), each topic backed by one or more speaking
-- sections, in turn backed by a reusable pool of sample sentences (text + IPA + sample audio) a
-- learner practices reading aloud, gated per learner via a persisted topic-progress state machine
-- identical in shape to listening_topic_progress/grammar_topic_progress.
...
INSERT INTO speaking_library_topics (code, name, description, level, sequence_order) VALUES
...
-- A topic's speaking content: one or more sections, each backed by its own reusable sample-sentence
-- pool (mirrors listening_library_sections' passage+question-pool shape, adjusted for per-sentence
-- sample audio instead of one passage-level audio + MCQ pool).
...
-- Persisted topic-gating state machine, structurally identical to listening_topic_progress/
-- grammar_topic_progress (see SpeakingTopicProgressMapper.xml): status is a plain VARCHAR (the enum
-- mapping/validation lives in Java, not the schema), and (user_id, topic_id) is unique so the
-- mapper's ON CONFLICT upserts work.
...
-- One row per scored sentence-attempt (unlike listening's one-row-per-whole-section-submission),
-- since speaking scores one sentence at a time as the learner records each one.
