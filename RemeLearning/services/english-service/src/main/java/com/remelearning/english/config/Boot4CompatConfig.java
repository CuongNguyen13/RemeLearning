...
public class Boot4CompatConfig {
	...
	public RestClient.Builder restClientBuilder(ObjectMapper objectMapper) {
		// Explicitly register Jackson's MappingJackson2HttpMessageConverter so that
		// .body(someRecord) serializes correctly to JSON — without this the RestClient sends
		// an empty body, causing FastAPI/Pydantic to reject with 422 "Field required" at ["body"].
		// (Boot 4.1.0's HttpMessageConvertersAutoConfiguration no longer auto-registers a
		// RestClient.Builder bean; RestClientConfig applies converters only through a
		// RestClientBuilderConfigurer, which requires a Spring-built builder.)
		...
		// ["body"], exactly the bug above but at the transport layer instead of the converter
		// layer. SimpleClientHttpRequestFactory fully buffers the request body before sending,
		// avoiding that streaming bug.
