...
| GET | `/api/v1/learners/{userId}/weak-points` | composite — fans out to english-service's 4 domains (vocabulary/grammar/pronunciation/listening) |
| GET | `/api/v1/learners/{userId}/recommendations` | proxy → recommendation-service |
| POST/GET | `/api/v1/learners/{userId}/practice/sessions[...]` | thin proxy → english-service's `practice.session` package (the refactored "Luyện tập"): start a session of ~4 AI exercises across random topics/skills, read one session, `sessions/latest` (resume), and `sessions/{sessionId}/exercises/{order}/complete` |
...
| GET/POST | `/api/v1/learners/{userId}/learn/{vocabulary\|grammar\|listening\|speaking}/library/*` | thin proxy → english-service's `*.library` packages (fixed 60-topic catalog + LOCKED/UNLOCKED/IN_PROGRESS/PASSED gating per domain; listening/speaking add AI-generated passage+audio or sample-sentence Sections; speaking's `sentences/{sentenceId}/attempts` is multipart, streamed via `FilePart` like the non-library speaking attempt proxy) |
| POST | `/api/v1/learners/{userId}/learn/{grammar\|listening\|speaking}/history/{attemptId}/ai-practice` | thin proxy → english-service's "generate practice from one past learn attempt's mistakes" endpoint per domain (`404` if the attempt isn't owned by `userId`) |
| POST | `/api/v1/learners/{userId}/learn/{grammar\|listening\|speaking}/library/sessions-or-sections/{id}/ai-practice` | thin proxy → english-service's "generate practice from one past library session/section's mistakes" endpoint per domain (path segment is `sessions` for grammar, `sections` for listening/speaking, matching each domain's own library route shape; `404` if not owned by `userId`) |
| GET | `/api/v1/learners/{userId}/learn/{grammar\|listening\|speaking}/merged-history` | thin proxy → english-service's merged (learn + library) history endpoint per domain, one time-sorted list tagged by `source` (`"LEARN"`/`"LIBRARY"`) |
