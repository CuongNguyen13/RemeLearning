...
| `RecommendationGeneratedEvent` | `{eventId, eventType, occurredAt, recordingId, userId, recommendations: [{itemId, category, label, recommendationText, forgettingScore}]}` | decoded from recommendation-service's camelCase JSON (BaseEvent envelope) via a plain `ObjectMapper` + `JavaTimeModule`, not `EventCodec` |
| `recent_recommendations` row | `{id, user_id, item_id, category, label, recommendation_text, forgetting_score, received_at}` | upserted on `(user_id, item_id)` — re-recommending the same item refreshes it in place |
...
- `recommendation.generated` is published by `recommendation-service`, a sibling Java service built
  concurrently with this one; as of this writing it has no producer wired up yet, but the topic
  constant and this consumer are both ready.
