...
below as a separate hop per `vocabulary-library.md`'s convention — as of this change `bff-service`
does not yet proxy these four endpoints (unlike Vocabulary/Grammar Library), a gap noted below.
...
- **Gap surfaced while documenting this flow:** `bff-service` does not yet proxy any of these four
  endpoints — unlike Vocabulary Library and Grammar Library, which both have a `LearnerController`
  pass-through, the FE cannot reach Listening Library through `bff-service` yet. Follow-up work,
  analogous to the vocabulary-library BFF-proxy commit, is needed before the FE can consume this API.
