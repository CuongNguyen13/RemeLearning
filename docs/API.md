...
> đối chiếu với `vocabulary-service/openapi.yaml`. Cập nhật tài liệu này khi thêm/sửa endpoint hoặc topic.

## Mục lục

- [1. vocabulary-service (Java) — REST](#1-vocabulary-service-java--rest)
- [2. ai-service (Python) — REST](#2-ai-service-python--rest)
- [3. Kafka — vocabulary-service (consumer)](#3-kafka--vocabulary-service-consumer)
- [4. Kafka — ai-service (consumer/producer)](#4-kafka--ai-service-consumerproducer)
- [5. Bảng tổng hợp](#5-bảng-tổng-hợp)
- [6. Các service chưa có API](#6-các-service-chưa-có-api)

---

## 1. `vocabulary-service` (Java) — REST
...
Spec đầy đủ (OpenAPI): `RemeLearning/services/vocabulary-service/openapi.yaml`. Swagger UI khi service chạy:
...
## 3. Kafka — `vocabulary-service` (consumer)
...
- File: `vocabulary-service/.../kafka/TranscriptReadyConsumer.java`
...
- File: `vocabulary-service/.../kafka/LearningGapAnalyzedConsumer.java`
...
- Xử lý: `VocabularyWeakPointService.saveWeakPoints(event)` — chỉ giữ item có `category == "vocabulary"`
  (payload chứa cả grammar/pronunciation, các category khác bị bỏ qua vì thuộc service tương lai khác);
...
| `pronunciation.analyzed` | pronunciation-service (dự kiến) | — | ⚠️ chỉ tồn tại tên hằng số, chưa có producer/consumer |
| `grammar.analyzed` | grammar-service (dự kiến) | — | ⚠️ chỉ tồn tại tên hằng số, chưa có producer/consumer |
| `vocabulary.analyzed` | vocabulary-service (dự kiến) | — | ⚠️ chỉ tồn tại tên hằng số, chưa có producer/consumer |
...
|---|---|---|---|---|
| vocabulary-service | REST | GET | `/api/v1/transcripts/{recordingId}` | 404 nếu chưa có transcript |
| vocabulary-service | REST | GET | `/api/v1/vocabulary/weak-points/{userId}` | filter `?type=` tùy chọn |
| vocabulary-service | REST | GET | `/api/v1/vocabulary/weak-points/{userId}/grouped` | nhóm theo `VocabularyType` |
...
| vocabulary-service | Kafka in | `transcript.ready` | `TranscriptReadyConsumer` | lưu Transcript + Segments |
| vocabulary-service | Kafka in | `learning.gap.analyzed` | `LearningGapAnalyzedConsumer` | lưu weak points (chỉ category vocabulary) |
...
- `pronunciation-service` (8083)
- `grammar-service` (8084)
- `recommendation-service` (8086)
- `dashboard-service` (8087)

Khi một trong các service trên được triển khai, nên theo mẫu của `vocabulary-service` (xem `CLAUDE.md` — mục
"vocabulary-service (Java) — reference implementation") và bổ sung phần tương ứng vào tài liệu này.
