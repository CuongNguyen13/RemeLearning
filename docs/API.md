...
câu hỏi không kèm đáp án khi trả về đề (`VocabQuestionDto`), chỉ tiết lộ đáp án sau khi chấm
(`VocabAttemptQuestionResultDto`).
...
- **Response `data`** — `VocabPracticeItemDto`: `{practiceItemId, level, examType, topic, targetWords[],
  ...
  (`type` — `VocabQuestionType`; không có field đáp án).

### GET `/api/v1/learn/vocabulary/items/{itemId}`

Chi tiết một bộ đề đã sinh (không đáp án) — `VocabPracticeItemDto` như trên. **Lỗi**: `404` nếu không tồn tại.
...
- **Response `data`** — `SectionCardDto`: `{sectionId, cardKind (INTRO|QUIZ), libraryWordId, word?
  (chỉ có ở INTRO), meaningVi?, exampleEn?, audioUrl?, exerciseType? (MCQ|CLOZE|MATCHING|
...
`GrammarAttemptDetailDto`) nhưng field giống 1-1.
...
- **Response `data`** — `ListeningPracticeItemDto`: `{practiceItemId, audioUrl (null nếu Supertonic
  ...
  options[]}` (`type` — `MCQ | KEYWORD | OPEN`).

### GET `/api/v1/learn/listening/items/{itemId}`

Chi tiết một bài đã sinh (không transcript/đáp án) — `ListeningPracticeItemDto` như trên. **Lỗi**: `404`.
...
| english-service (vocabulary.learn) | REST | GET | `/api/v1/learn/vocabulary/items/{itemId}` | chi tiết bộ đề (không đáp án); `404` |
...
| english-service (grammar.learn) | REST | GET | `/api/v1/learn/grammar/items/{itemId}` | chi tiết bộ đề (không đáp án); `404` |
...
| english-service (listening) | REST | GET | `/api/v1/learn/listening/items/{itemId}` | chi tiết bài (không transcript/đáp án); `404` |
