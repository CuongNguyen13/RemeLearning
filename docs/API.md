...
`english-service` nay có thêm **4 domain "Học &amp; Luyện tập với AI"** hoàn chỉnh — mỗi kỹ năng học
(vocabulary/grammar/listening/speaking) có cùng một shape: `POST /{userId}/generate` sinh một bộ luyện
tập bằng AI (nhắm vào `focusItems` truyền vào, hoặc nếu bỏ trống thì lấy top weak point hiện tại của
learner), `GET /items/{itemId}` + `GET /{userId}/items` để xem lại đề đã sinh, `POST /attempts` để chấm
điểm, `GET /history/{userId}` + `GET /history/{userId}/{attemptId}` để xem lịch sử. `vocabulary` sống
trong package mới `vocabulary.learn` (song song `vocabulary` cũ, vốn chỉ có weak-points/transcript),
tương tự `grammar.learn`; `listening`/`speaking` là hai package đỉnh mới hoàn toàn
(`com.remelearning.english.listening`, `com.remelearning.english.speaking`), không nằm dưới domain nào
...
Speaking còn gọi sang **ai-service** để chấm phát âm: `PronunciationScoringClient` (`common.ai.pronunciation`)
gọi `POST /api/v1/pronunciation/score` (mục 7, GOP wav2vec2) — audio mẫu (Supertonic) và audio chấm điểm
đều đi qua `StorageClient` như dictation. Xem chi tiết cả 4 domain ở mục 1 (endpoint) và mục 6 (proxy
bff-service).
...
|---|---|---|
| `itemId` | string | |
| `category` | string | `vocabulary \| grammar \| pronunciation \| listening` (mistake_history cập nhật cho cả 4 category, kể cả `listening` dù category đó không có bảng weak-point riêng — xem đầu mục 1) |
| `label` | string | |
...
### GET `/api/v1/dictation/folders/{folderId}/lessons/{userId}` (rev 3: thêm `{userId}`, join tiến độ)

Danh sách bài tập (clip) trong 1 folder — nhẹ, **không** có `scriptText`/`sentences`, chỉ đủ để render
danh sách; script chỉ tải khi mở chi tiết 1 bài (endpoint dưới). Mỗi bài được join với tiến độ riêng
của `userId` trên bài đó.
- **Response `data`** — `DictationLessonSummaryDto[]`: `{clipId, code, title, audioUrl, level,
  sentenceCount, attemptCount, latestAccuracy}`. `sentenceCount` thay cho thời lượng (hệ thống không
  lưu độ dài audio). `attemptCount`/`latestAccuracy` (0..1, độ chính xác lần luyện gần nhất) là `null`
  nếu learner chưa luyện bài này lần nào.
...
- **Xử lý** (`DictationServiceImpl`):
  ...
  3. Gọi `DictationAnalyzer` (Gemini nếu `dictation.analyzer.mode=llm`, mặc định rule-based) — phân
     tích căn nguyên lỗi (root-cause: LEXICON/GRAMMAR/PHONOLOGY) từ toàn bộ `referenceText` +
     `userTranscript` + diff, trả về bảng đối chiếu lỗi (`errorTable`), tóm tắt căn nguyên theo từng
     nhóm có lỗi (`rootCauses`), lời khuyên hành động (`actionAdvice`) và câu luyện tập
     (`practiceSentences`, lưu vào `dictation_practice_items`).
  4. Publish `learning.gap.analyzed` (các từ sai thành `WeakPointPayload` category `vocabulary`,
     `forgettingScore` bão hòa theo số lần sai, `recommendation` lấy từ `actionAdvice[0]`) để pipeline
     gợi ý xử lý.
- **Response `data`** — `DictationAttemptResultDto`: `{referenceText, accuracy, wer, diff[],
  errorTable[], rootCauses[], actionAdvice[], practiceSentences[]}`; `accuracy`/`wer`/`diff` vẫn
  tính trên `userTranscript`, không tính `sentenceMistakes`. `errorTable`:
  `[{original, transcribed, category, note}]`; `rootCauses`: `[{category, summary, examples[]}]`
  (chỉ chứa nhóm thực sự có lỗi).
...
sai (lấy thẳng từ `dictation_misses`, không tái tạo diff theo vị trí — xem ghi chú bên dưới), và
phân tích căn nguyên AI đã sinh ra tại thời điểm đó (cột `ai_suggestions` trên `dictation_attempts` —
tên cột giữ nguyên từ trước, nhưng nay lưu JSON của cả `errorTable`/`rootCauses`/`actionAdvice`
thay vì chỉ một mảng string; các attempt cũ hơn tự động đọc lại thành `actionAdvice` rỗng
`errorTable`/`rootCauses`, không lỗi).
- **Response `data`** — `DictationAttemptDetailDto`: `{attemptId, title, skill, level, examType,
  ...
  errorTable[], rootCauses[], actionAdvice[], attemptedAt}`.
...
### Vocabulary Learn — luyện từ vựng với AI (package `vocabulary.learn`)

Sinh bộ đề luyện tập (cloze/MCQ/matching-theo-nghĩa) bằng AI, tách biệt hoàn toàn với package
`vocabulary` cũ (chỉ có transcript/weak-points). Bộ đề generate lần nào lưu lần đó (`vocab_practice_items`).
Câu hỏi (`VocabQuestionDto`) **nay kèm đáp án** (`answer`) + `translation` để FE chấm từng câu ngay trên
giao diện (phản hồi tức thì, mirror `VocabAttemptScorer`/`ExactMatchScorer`); điểm chính thức vẫn đến từ
endpoint submit attempt (`VocabAttemptQuestionResultDto`).

### POST `/api/v1/learn/vocabulary/{userId}/generate`

Sinh một bộ đề luyện từ vựng mới.
- **Path param**: `userId` (string)
- **Request body** (tùy chọn, có thể để trống) — `GenerateVocabPracticeRequest`: `{level?, examType?,
  focusItems?: string[]}`. `focusItems` có giá trị thì ưu tiên dùng đúng các từ đó; nếu bỏ trống, service
  lấy top 8 vocabulary weak point hiện tại của `userId` (`VocabularyWeakPointService.getTopWeakPoints`);
  nếu learner chưa có weak point nào, generator tự chọn từ theo `level`/`examType`.
- **Response `data`** — `VocabPracticeItemDto`: `{practiceItemId, level, examType, topic, targetWords[],
  questions: VocabQuestionDto[], createdAt}`. `VocabQuestionDto`: `{index, prompt, type, options[],
  answer, translation}` (`type` — `VocabQuestionType`; `answer` = đáp án đúng dùng cho FE chấm local).

### GET `/api/v1/learn/vocabulary/items/{itemId}`

Chi tiết một bộ đề đã sinh (nay kèm `answer`/`translation` để FE chấm local) — `VocabPracticeItemDto` như
trên. **Lỗi**: `404` nếu không tồn tại.

### GET `/api/v1/learn/vocabulary/{userId}/items`

Danh sách bộ đề đã sinh của một learner, mới nhất trước — `VocabPracticeItemDto[]`.

### POST `/api/v1/learn/vocabulary/attempts`

Chấm điểm một lần làm bài.
- **Request body** — `SubmitVocabAttemptRequest`: `userId` (bắt buộc), `practiceItemId` (bắt buộc),
  `answers: string[]` (bắt buộc, khớp theo index với `questions`).
- **Xử lý** (`VocabLearnServiceImpl`): chấm bằng `VocabAttemptScorer` (pure, so khớp từng câu), lưu
  `vocab_practice_attempts`, rồi gọi thẳng `PracticeService.redo(...)` — mỗi từ mục tiêu duy nhất
  (khử trùng lặp) thành một `PracticeAttemptRequest` (`itemId="vocab:<word>"`,
  `category=LearningCategories.VOCABULARY`) — tái dùng nguyên `WeakPointScoringOrchestrator`/
  `mistake_history`/`AnalysisRequestedProducer` của mục "Practice / redo-exercise" phía trên, không có
  publisher riêng cho domain learn này.
- **Response `data`** — `VocabAttemptResultDto`: `{accuracy, results: VocabAttemptQuestionResultDto[],
  actionAdvice: string[]}`. `VocabAttemptQuestionResultDto`: `{index, prompt, yourAnswer, correctAnswer,
  correct, translation}`. `actionAdvice` — tiếng Việt, một dòng cho mỗi từ sai (không trùng lặp).

### GET `/api/v1/learn/vocabulary/history/{userId}`

Lịch sử các lần làm bài, mới nhất trước — `VocabAttemptHistoryEntryDto[]`: `{attemptId, practiceItemId,
level, examType, topic, score, attemptedAt}`.

### GET `/api/v1/learn/vocabulary/history/{userId}/{attemptId}`

Chi tiết một lần làm bài trong lịch sử — `VocabAttemptDetailDto`: `{attemptId, level, examType, topic,
accuracy, results: VocabAttemptQuestionResultDto[], attemptedAt}`. **Lỗi**: `404` nếu không tồn tại
hoặc không thuộc về `userId`.

### Vocabulary Library — thư viện từ vựng theo chủ đề + Section (package `vocabulary.library`)

Thư viện từ vựng cố định theo chủ đề (topic), học theo từng "Section" (đợt 5–20 từ), tách biệt hoàn
toàn với `vocabulary.learn` (generate đề rời rạc mỗi lần) và `vocabulary` cũ (transcript/weak-points).
Mỗi Section gồm card INTRO (giới thiệu từ mới) xen kẽ card QUIZ (MCQ/CLOZE/MATCHING/
LISTENING_DICTATION/TRANSLATE_EN_TO_VI/TRANSLATE_VI_TO_EN); chấm xong feed vào pipeline weak-point/
spaced-repetition sẵn có giống các domain "Học &amp; Luyện tập" khác.

### GET `/api/v1/learn/vocabulary/library/{userId}/topics`

Danh sách mọi chủ đề, kèm số từ trong chủ đề và số từ learner đã "mastered".
- **Path param**: `userId` (string)
- **Response `data`** — `TopicSummaryDto[]`: `{topicId, code, name, description?, level?, wordCount,
  masteredCount}`.

### POST `/api/v1/learn/vocabulary/library/{userId}/topics/{topicId}/sections`

Bắt đầu một Section mới cho một chủ đề (tự động nạp thêm từ qua LLM nếu chủ đề chưa đủ từ), trả về
card đầu tiên.
- **Path param**: `userId` (string), `topicId` (int64)
- **Request body** (tùy chọn) — `StartSectionRequest`: `{sectionSize?}` (kẹp server-side về `[5,20]`,
  mặc định 10).
- **Response `data`** — `SectionCardDto`: `{sectionId, cardKind (INTRO|QUIZ), libraryWordId, word?
  (chỉ có ở INTRO), ipa? (phiên âm IPA, chỉ có ở INTRO), meaningVi?, exampleEn?, audioUrl?, exerciseType? (MCQ|CLOZE|MATCHING|
  LISTENING_DICTATION|TRANSLATE_EN_TO_VI|TRANSLATE_VI_TO_EN), prompt?, options?: string[],
  progress: {totalWords, wordsMastered, wordsRemaining}}`.

### POST `/api/v1/learn/vocabulary/library/sections/{sectionId}/answers`

Chấm câu trả lời hiện tại (hoặc chỉ ghi nhận đã xem nếu đang là card INTRO) và trả về card kế tiếp,
hoặc kết quả hoàn thành nếu đã hết Section.
- **Path param**: `sectionId` (int64)
- **Request body** (tùy chọn) — `SubmitSectionAnswerRequest`: `{submittedAnswer?}` (bỏ trống/rỗng nếu
  đang ở card INTRO).
- **Response `data`** — `SectionAnswerResultDto`: `{correct, correctAnswer?, score, completed,
  nextCard?: SectionCardDto, progress}`.

### POST `/api/v1/learn/vocabulary/library/sections/{sectionId}/finish`

Kết thúc sớm một Section đang dở; các câu đã trả lời vẫn được feed vào pipeline weak-point.
- **Path param**: `sectionId` (int64)
- **Response `data`** — `SectionAnswerResultDto` (giống endpoint trên).

### GET `/api/v1/learn/vocabulary/library/{userId}/sections/history`

Lịch sử các Section đã hoàn thành của một learner, mới nhất trước.
- **Path param**: `userId` (string)
- **Response `data`** — `SectionHistoryEntryDto[]`: `{sectionAttemptId, topicName?, accuracy,
  wordsCount, completedAt?}`.

### GET `/api/v1/learn/vocabulary/library/words/{wordId}/audio`

Stream file audio phát âm đã tổng hợp cho một từ trong thư viện.
- **Path param**: `wordId` (int64)
- **Response**: `audio/wav` binary.

### Grammar Learn — luyện ngữ pháp với AI (package `grammar.learn`)

Cấu trúc, endpoint và DTO **giống hệt** Vocabulary Learn ở trên (bản sao cấu trúc — error-
correction/fill-tense/transform/MCQ thay cho cloze/MCQ/matching), chỉ đổi `category` thành
`LearningCategories.GRAMMAR` và `itemId` prefix `"grammar:"`. Base path `/api/v1/learn/grammar`, cùng 6
endpoint (`{userId}/generate`, `items/{itemId}`, `{userId}/items`, `attempts`, `history/{userId}`,
`history/{userId}/{attemptId}`) với DTO tương ứng đổi tên (`GenerateGrammarPracticeRequest`,
`GrammarPracticeItemDto`, `GrammarQuestionDto`, `SubmitGrammarAttemptRequest`,
`GrammarAttemptResultDto`, `GrammarAttemptQuestionResultDto`, `GrammarAttemptHistoryEntryDto`,
`GrammarAttemptDetailDto`) nhưng field giống 1-1. Như Vocabulary, `GrammarQuestionDto` **nay kèm
`answer` + `translation`** để FE chấm từng câu local (điểm chính thức vẫn từ endpoint submit).
...
**cố định** 60 chủ điểm ngữ pháp (beginner → intermediate → advanced, seed cứng trong migration), mỗi
chủ điểm có một trang lý thuyết (giải thích EN/VI + minh họa cấu trúc câu + ví dụ) và một bộ 8-10 câu
hỏi luyện tập, **sinh bằng AI một lần duy nhất** rồi tái sử dụng mãi mãi (giống cách
`vocabulary.library` top-up từ vựng theo topic). Learner đi tuần tự: chủ điểm đầu tiên luôn
`UNLOCKED`, các chủ điểm sau `LOCKED` cho tới khi chủ điểm liền trước được `PASSED` (làm đúng hết một
session, kể cả sau khi làm lại các câu sai). Câu sai được sinh lại một câu AI mới (không lặp câu cũ)
để làm `RETRY` session, không đụng vào pool câu hỏi dùng chung. Chấm xong luôn feed vào pipeline
weak-point/spaced-repetition sẵn có qua `PracticeService.redo` (`itemId = "grammar:" + topicCode`,
category `LearningCategories.GRAMMAR`), giống mọi domain "Học &amp; Luyện tập" khác.

### GET `/api/v1/learn/grammar/library/{userId}/topics`

Danh sách toàn bộ 60 chủ điểm kèm trạng thái tiến độ của learner (tự bootstrap chủ điểm đầu tiên
thành `UNLOCKED` nếu learner chưa có row nào).
- **Path param**: `userId` (string)
- **Response `data`** — `GrammarLibraryTopicDto[]`: `{topicId, code, name, description?, level,
  sequenceOrder, status (LOCKED|UNLOCKED|IN_PROGRESS|PASSED)}`.

### GET `/api/v1/learn/grammar/library/topics/{topicId}`

Trang lý thuyết + pool câu hỏi của một chủ điểm; nếu chưa có (lần đọc đầu tiên), sinh bằng AI rồi lưu
lại trong cùng 1 transaction — các lần đọc sau chỉ đọc từ DB, không gọi AI nữa.
- **Path param**: `topicId` (int64)
- **Response `data`** — `GrammarLibraryContentDto`: `{topicId, explanationEn, explanationVi,
  illustrationText (bảng markdown hoặc mermaid diagram mô tả công thức câu), examples:
  {en, vi}[], questions: GrammarLibraryQuestionDto[]}`. `GrammarLibraryQuestionDto` **có kèm
  `answer`/`explanationVi`/`translationVi`** vì đây là trang đọc lý thuyết, không phải bài thi (khác
  với `GrammarSessionQuestionDto` dùng khi đang làm session). `translationVi` là bản dịch nghĩa tiếng
  Việt thuần túy của câu `answer`, khác với `explanationVi` (giải thích quy tắc ngữ pháp).
- **Lỗi**: `404` nếu `topicId` không tồn tại.

### POST `/api/v1/learn/grammar/library/{userId}/topics/{topicId}/sessions`

Bắt đầu một session `INITIAL` mới cho chủ điểm (lấy toàn bộ pool câu hỏi), chuyển trạng thái chủ điểm
sang `IN_PROGRESS`.
- **Path param**: `userId` (string), `topicId` (int64)
- **Response `data`** — `StartGrammarSessionResponse`: `{sessionId, sessionType (INITIAL|RETRY),
  questions: GrammarSessionQuestionDto[], totalCount}`. `GrammarSessionQuestionDto` **không** kèm
  `answer`/`explanationVi` (đang làm bài, không lộ đáp án).
- **Lỗi**: `403` nếu chủ điểm đang `LOCKED` cho learner này.

### POST `/api/v1/learn/grammar/library/sessions/{sessionId}/answers`

Chấm một câu trả lời trong session đang dở (so khớp chính xác qua `ExactMatchScorer`, dùng chung với
`grammar.learn`).
- **Path param**: `sessionId` (int64)
- **Request body** — `SubmitGrammarLibraryAnswerRequest`: `{questionRef, submittedAnswer?}`.
- **Response `data`** — `GrammarLibraryAnswerResultDto`: `{questionRef, correct, correctAnswer?,
  explanationVi?, translationVi?}`.
- **Lỗi**: `400` nếu `questionRef` không thuộc session; `409` nếu session đã `COMPLETED`.

### POST `/api/v1/learn/grammar/library/sessions/{sessionId}/finish`

Kết thúc session: tính lại toàn bộ câu sai (câu chưa trả lời tính là sai), feed **mọi** câu hỏi trong
session vào pipeline weak-point. Nếu còn câu sai → sinh một session `RETRY` mới chỉ gồm các câu sai đã
được AI tạo lại (khác câu cũ). Nếu hết câu sai → chủ điểm chuyển `PASSED`, tự mở khóa (`UNLOCKED`) chủ
điểm kế tiếp theo `sequenceOrder` (nếu đang `LOCKED` hoặc chưa có row).
- **Path param**: `sessionId` (int64)
- **Response `data`** — `FinishGrammarLibrarySessionResponse`: `{sessionId, correctCount, totalCount,
  passed, retrySession?: StartGrammarSessionResponse, nextTopicUnlocked, nextTopicId?}`.
- **Lỗi**: `409` nếu session đã `COMPLETED`.

### GET `/api/v1/learn/grammar/library/{userId}/topics/{topicId}/history`
...
Chi tiết một bài đã sinh (không transcript; nay kèm `answer` MCQ/KEYWORD + `explanation` để FE chấm local,
`answer` null với OPEN) — `ListeningPracticeItemDto` như trên. **Lỗi**: `404`.
...
Chấm điểm một lần làm bài.
- **Request body** — `SubmitListeningAttemptRequest`: `userId`, `practiceItemId`, `answers: string[]`
  (bắt buộc cả ba, khớp theo index với `questions`).
- **Xử lý** (`ListeningLearnServiceImpl`): câu `MCQ`/`KEYWORD` chấm bằng `ListeningQuestionScoring`
  (exact-match / WER, thuần túy); câu `OPEN` chấm bằng `OpenAnswerGrader` (LLM, so với transcript +
  đáp án mẫu). Mỗi câu ra `subScore` (0..1); `correct = subScore >=
  ListeningQuestionScoring.CORRECT_THRESHOLD`; `accuracy` = trung bình `subScore` toàn bài. Sau đó gọi
  `PracticeService.redo(...)` — câu `KEYWORD` dùng `answer` làm `label`/`itemId` (category
  `LearningCategories.LISTENING`), câu MCQ/OPEN dùng `question.skill` — như lưu ý ở đầu mục 1,
  category `listening` hiện **không** được `WeakPointDispatcherImpl` ghi vào bảng nào (chỉ cập nhật
  `mistake_history`).
...
Lịch sử các lần làm bài, mới nhất trước — `ListeningAttemptHistoryEntryDto[]`: `{attemptId,
practiceItemId, level, examType, topic, score, attemptedAt}`.

### GET `/api/v1/learn/listening/history/{userId}/{attemptId}`
...
(Goodness-of-Pronunciation) qua ai-service (`common.ai.pronunciation.PronunciationScoringClient` →
`POST /api/v1/pronunciation/score`, mục 7). Khác Vocabulary/Grammar/Listening: hiển thị **sẵn** đề bài
(`targetText`) ngay từ khi generate — luyện nói là phát âm đúng văn bản đã biết, không phải kiểm tra
hiểu nội dung.

### POST `/api/v1/learn/speaking/{userId}/generate`

Sinh một câu/đoạn luyện nói mới kèm audio mẫu (Supertonic, tổng hợp đồng bộ trong request).
- **Request body** (tùy chọn) — `GenerateSpeakingPracticeRequest`: `{level?, examType?, focusItems?:
  string[]}`. `focusItems` rỗng → lấy top 8 pronunciation weak point hiện tại của learner
  (`PronunciationWeakPointService.getWeakPoints`, sắp theo `forgettingScore`); rỗng hoàn toàn →
  generator tự chọn câu theo `level`/`examType`.
- **Response `data`** — `SpeakingPracticeItemDto`: `{practiceItemId, sampleAudioUrl (null nếu chưa
  tổng hợp xong), level, examType, topic, targetText, translation, createdAt}`.

### GET `/api/v1/learn/speaking/items/{itemId}`

Chi tiết một bài đã sinh (`targetText` + URL audio mẫu) — `SpeakingPracticeItemDto` như trên. **Lỗi**: `404`.

### GET `/api/v1/learn/speaking/{userId}/items`

Danh sách bài đã sinh của một learner, mới nhất trước — `SpeakingPracticeItemDto[]`.

### GET `/api/v1/learn/speaking/items/{itemId}/sample-audio`

Stream audio mẫu Supertonic của một bài (đọc từ `StorageClient`, `audio/wav`). **Lỗi**: `404` nếu audio
mẫu chưa tổng hợp xong.

### POST `/api/v1/learn/speaking/{userId}/attempts` (multipart)

Nộp bản ghi âm của learner để chấm điểm.
- **Path param**: `userId` (string)
- **Request** (multipart form): `practiceItemId` (Long, bắt buộc), `audio` (file, bắt buộc)
- **Xử lý** (`SpeakingLearnServiceImpl`): lưu audio learner vào `StorageClient` (key
  `speaking/attempts/{userId}/{uuid}.wav`), gọi `PronunciationScoringClient.score(...)` (→ ai-service
  `POST /api/v1/pronunciation/score`, mục 7) với `expected_text = targetText` của practice item, lưu
  kết quả (`speaking_attempts`), rồi gọi `PracticeService.redo(...)` — mỗi từ duy nhất thành một
  `PracticeAttemptRequest` (`itemId="pronunciation:<word>"`, category
  `LearningCategories.PRONUNCIATION` — **tái dùng** category `pronunciation` đã có sẵn, không tạo
  category mới cho speaking) với `correct = (score >= 0.6)` (nhị phân hoá điểm liên tục GOP).
- **Response `data`** — `SpeakingAttemptResultDto`: `{overall, words: WordScoreDto[], transcript,
  weakPhonemes: string[], actionAdvice: string[]}`. `WordScoreDto`: `{word, score, phonemes:
  PhonemeScoreDto[]}`; `PhonemeScoreDto`: `{ipa, score}`. `transcript` là bản ASR (Whisper, không phải
  chấm điểm) của chính bản ghi âm learner, để đối chiếu với `targetText`.

### GET `/api/v1/learn/speaking/history/{userId}`

Lịch sử các lần làm bài, mới nhất trước — `SpeakingAttemptHistoryEntryDto[]`: `{attemptId,
practiceItemId, level, examType, topic, overallScore, attemptedAt}`.

### GET `/api/v1/learn/speaking/history/{userId}/{attemptId}`

Chi tiết một lần làm bài — `SpeakingAttemptDetailDto`: `{attemptId, level, examType, topic, targetText,
...
- **Path param**: `userId` (string)
- **Query param** (tùy chọn): `category` — `"vocabulary" | "grammar" | "pronunciation" | "listening"` (string thô, không phải enum riêng — giá trị đến trực tiếp từ `WeakPointPayload.category` của `learning.gap.analyzed`; `"listening"` chỉ xuất hiện ở đây nếu request thật sự đi qua vòng Kafka `learning.gap.analysis.requested` → `learning.gap.analyzed` — xem ghi chú category `listening` ở đầu mục 1, không có bảng `listening_weak_points` riêng ở `english-service`)
...
| `itemId` | string | |
| `category` | string | `vocabulary`/`grammar`/`pronunciation`/`listening` |
| `label` | string | |
...
### Learn skills (vocabulary/grammar/listening/speaking) — proxy sang `english-service` (mục 1)

Tất cả thin proxy sang 4 domain "learn" của `english-service`, base
`/api/v1/learners/{userId}/learn/<skill>/...`. Với `attempts` (trừ speaking, multipart), `userId` trong
path được gán đè lên body (`request.setUserId(userId)`) trước khi proxy, giống pattern
`practice/redo` — client không cần lặp lại `userId` trong body.

> **Lưu ý contract mới:** các response `VocabPracticeItemDto`/`GrammarPracticeItemDto`/
> `ListeningPracticeItemDto` (generate + items + item detail) nay proxy nguyên cả field đáp án mới —
> `answer`/`translation` (vocab, grammar) và `answer`/`explanation` (listening; `answer` null với câu
> `OPEN`) — để FE chấm từng câu local (phản hồi tức thì). Điểm chính thức vẫn đến từ endpoint submit
> attempt.

- **POST `/api/v1/learners/{userId}/learn/vocabulary/generate`** (body
  `GenerateVocabPracticeRequestDto?`) → `VocabPracticeItemDto`.
- **GET `/api/v1/learners/{userId}/learn/vocabulary/items`** → `VocabPracticeItemDto[]`.
- **GET `/api/v1/learners/{userId}/learn/vocabulary/items/{itemId}`** → `VocabPracticeItemDto`.
- **POST `/api/v1/learners/{userId}/learn/vocabulary/attempts`** (body `SubmitVocabAttemptRequestDto`) →
  `VocabAttemptResultDto`.
- **GET `/api/v1/learners/{userId}/learn/vocabulary/history`** → `VocabAttemptHistoryEntryDto[]`.
- **GET `/api/v1/learners/{userId}/learn/vocabulary/history/{attemptId}`** → `VocabAttemptDetailDto`.
...
- **GET `/api/v1/learners/{userId}/learn/listening/history/{attemptId}`** → `ListeningAttemptDetailDto`.
- **POST `/api/v1/learners/{userId}/learn/speaking/generate`** (body
  `GenerateSpeakingPracticeRequestDto?`) → `SpeakingPracticeItemDto`.
- **GET `/api/v1/learners/{userId}/learn/speaking/items`** → `SpeakingPracticeItemDto[]`.
- **GET `/api/v1/learners/{userId}/learn/speaking/items/{itemId}`** → `SpeakingPracticeItemDto`.
- **GET `/api/v1/learners/{userId}/learn/speaking/items/{itemId}/sample-audio`** → relay stream audio.
- **POST `/api/v1/learners/{userId}/learn/speaking/attempts`** (multipart `audio` + `practiceItemId`,
  streamed straight through via `FilePart`, không buffer file trong bff-service) → `SpeakingAttemptResultDto`.
...
### Grammar Library (BFF proxy) — proxy sang `english-service` (mục 1)

Thin proxy sang 6 endpoint `grammar.library` của `english-service`, base
`/api/v1/learners/{userId}/learn/grammar/library/...`. `userId` luôn có trong path theo convention
của controller này (kể cả với `topics/{topicId}`, `sessions/{sessionId}/answers`/`finish` mà bản thân
english-service không cần `userId` cho các endpoint đó) nhưng không được proxy tiếp xuống body/path
của english-service — giống hệt cách `vocabulary.library` đã làm ở trên. Các DTO là class riêng
trong `com.remelearning.bff.dto` (không dùng lại DTO/enum của english-service — các enum
`GrammarTopicStatus`/`GrammarSessionType`/`GrammarQuestionType` được map về `String` phẳng), field
giống 1-1: `GrammarLibraryTopicDto`, `GrammarLibraryContentDto`, `GrammarLibraryExampleDto`,
`GrammarLibraryQuestionDto`, `GrammarSessionQuestionDto`, `StartGrammarSessionResponseDto`,
`SubmitGrammarLibraryAnswerRequestDto`, `GrammarLibraryAnswerResultDto`,
`FinishGrammarLibrarySessionResponseDto`, `GrammarLibraryHistoryEntryDto`.

- **GET `/api/v1/learners/{userId}/learn/grammar/library/topics`** → `GrammarLibraryTopicDto[]`.
- **GET `/api/v1/learners/{userId}/learn/grammar/library/topics/{topicId}`** → `GrammarLibraryContentDto`.
- **POST `/api/v1/learners/{userId}/learn/grammar/library/topics/{topicId}/sessions`** →
  `StartGrammarSessionResponseDto`; `403` nếu chủ điểm `LOCKED`.
- **POST `/api/v1/learners/{userId}/learn/grammar/library/sessions/{sessionId}/answers`** (body
  `SubmitGrammarLibraryAnswerRequestDto?`) → `GrammarLibraryAnswerResultDto`.
- **POST `/api/v1/learners/{userId}/learn/grammar/library/sessions/{sessionId}/finish`** →
  `FinishGrammarLibrarySessionResponseDto`.
...
- **GET `/api/v1/learners/{userId}/dictation/folders/{folderId}/lessons`** (rev 3: nay truyền `userId`
  xuống english-service để join tiến độ) → `DictationLessonSummaryDto[]`
  `{clipId, code, title, audioUrl, level, sentenceCount, attemptCount, latestAccuracy}` (không script).
...
- **POST `/api/v1/learners/{userId}/dictation/attempts`** — `userId` gán đè lên body; body
  ...
  `{referenceText, accuracy, wer, diff[], errorTable[], rootCauses[], actionAdvice[], practiceSentences[]}`.
...
Chấm điểm phát âm của một bản ghi âm learner theo Goodness-of-Pronunciation (GOP) — dùng bởi
`english-service`'s speaking "learn" skill (mục 1, `SpeakingLearnServiceImpl`/`PronunciationScoringClient`).

- **Request** (multipart form): `audio` (`UploadFile`, bắt buộc), `expected_text` (string, bắt buộc —
  văn bản learner được yêu cầu đọc), `language_code` (string, mặc định `"en"`)
- **Xử lý**: `g2p_en` chuyển `expected_text` thành chuỗi phoneme ARPAbet; acoustic model
  (`app/pronunciation/gop_model.py`, mặc định `facebook/wav2vec2-lv-60-espeak-cv-ft`) suy ra xác suất
  phoneme theo từng frame; `app/pronunciation/gop_scorer.py` chấm mỗi phoneme kỳ vọng theo mức độ được
  các xác suất đó ủng hộ (GOP, có đơn giản hoá — xem docstring module). Song song, `transcript` được
  sinh bằng chính Whisper engine `/api/v1/upload` dùng (ASR thuần, không phải điểm chấm), để learner đối
  chiếu với những gì mình định nói.
- **Gate**: chỉ chạy khi `PRONUNCIATION_ENABLED=true` (`app/config.py`, mặc định `false` — tải acoustic
  model tốn RAM/CPU thật, không trả giá cho tới khi skill "Nói/Phát âm" thực sự bật). **Lỗi**: `503`
  (`"Pronunciation scoring is disabled (set PRONUNCIATION_ENABLED=true)"`) nếu tắt.
- **Response `data`** — `PronunciationScoreResponse` (`app/schemas/pronunciation.py`):

| Field | Type | Mô tả |
|---|---|---|
| `overall` | float | Điểm GOP trung bình toàn bộ phoneme |
| `words` | `WordScoreResponse[]` | `{word, score, phonemes: PhonemeScoreResponse[]}` — `phonemes`: `{ipa, score}` |
...
| bff-service | REST | POST | `/api/v1/learners/{userId}/learn/grammar/attempts` | proxy chấm điểm (`userId` gán đè lên body) |
| bff-service | REST | GET | `/api/v1/learners/{userId}/learn/grammar/history` | proxy lịch sử làm bài |
| bff-service | REST | GET | `/api/v1/learners/{userId}/learn/grammar/history/{attemptId}` | proxy chi tiết 1 lần làm bài |
| bff-service | REST | GET | `/api/v1/learners/{userId}/learn/grammar/library/topics` | proxy `GET /api/v1/learn/grammar/library/{userId}/topics` |
| bff-service | REST | GET | `/api/v1/learners/{userId}/learn/grammar/library/topics/{topicId}` | proxy trang lý thuyết + pool câu hỏi |
| bff-service | REST | POST | `/api/v1/learners/{userId}/learn/grammar/library/topics/{topicId}/sessions` | proxy bắt đầu session INITIAL |
| bff-service | REST | POST | `/api/v1/learners/{userId}/learn/grammar/library/sessions/{sessionId}/answers` | proxy chấm 1 câu trong session |
...
| bff-service | REST | POST | `/api/v1/learners/{userId}/learn/speaking/generate` | proxy `POST /api/v1/learn/speaking/{userId}/generate` |
| bff-service | REST | GET | `/api/v1/learners/{userId}/learn/speaking/items` | proxy danh sách bài đã sinh |
| bff-service | REST | GET | `/api/v1/learners/{userId}/learn/speaking/items/{itemId}` | proxy chi tiết 1 bài |
| bff-service | REST | GET | `/api/v1/learners/{userId}/learn/speaking/items/{itemId}/sample-audio` | relay stream audio mẫu Supertonic |
...
| bff-service | REST | GET | `/api/v1/learners/{userId}/dictation/folders/{folderId}/lessons` | proxy `/api/v1/dictation/folders/{folderId}/lessons/{userId}` (rev 3, join tiến độ) |
...
| english-service (vocabulary.learn) | REST | POST | `/api/v1/learn/vocabulary/attempts` | chấm điểm, feed weak-point qua `PracticeService.redo` |
| english-service (vocabulary.learn) | REST | GET | `/api/v1/learn/vocabulary/history/{userId}` | lịch sử làm bài |
| english-service (vocabulary.learn) | REST | GET | `/api/v1/learn/vocabulary/history/{userId}/{attemptId}` | chi tiết 1 lần làm bài; `404` |
| english-service (vocabulary.library) | REST | GET | `/api/v1/learn/vocabulary/library/{userId}/topics` | danh sách chủ đề + số từ + số từ đã mastered |
| english-service (vocabulary.library) | REST | POST | `/api/v1/learn/vocabulary/library/{userId}/topics/{topicId}/sections` | bắt đầu Section mới (top up từ qua LLM nếu cần), trả card đầu tiên |
| english-service (vocabulary.library) | REST | POST | `/api/v1/learn/vocabulary/library/sections/{sectionId}/answers` | chấm card hiện tại, trả card kế tiếp hoặc kết quả hoàn thành |
| english-service (vocabulary.library) | REST | POST | `/api/v1/learn/vocabulary/library/sections/{sectionId}/finish` | kết thúc sớm 1 Section, vẫn feed weak-point pipeline |
| english-service (vocabulary.library) | REST | GET | `/api/v1/learn/vocabulary/library/{userId}/sections/history` | lịch sử các Section đã hoàn thành |
| english-service (vocabulary.library) | REST | GET | `/api/v1/learn/vocabulary/library/words/{wordId}/audio` | stream audio phát âm của 1 từ trong thư viện |
| english-service (grammar.learn) | REST | POST | `/api/v1/learn/grammar/{userId}/generate` | sinh bộ đề luyện ngữ pháp bằng AI |
| english-service (grammar.learn) | REST | GET | `/api/v1/learn/grammar/items/{itemId}` | chi tiết bộ đề (nay kèm `answer`/`translation` cho FE chấm local); `404` |
| english-service (grammar.learn) | REST | GET | `/api/v1/learn/grammar/{userId}/items` | danh sách bộ đề đã sinh của learner |
...
| english-service (speaking) | REST | GET | `/api/v1/learn/speaking/items/{itemId}/sample-audio` | stream audio mẫu Supertonic; `404` nếu chưa xong |
...
| english-service (dictation) | REST | GET | `/api/v1/dictation/folders/{folderId}/lessons/{userId}` | (rev 3) danh sách bài trong 1 folder, không script, join tiến độ của `userId` (level/sentenceCount/attemptCount/latestAccuracy) |
