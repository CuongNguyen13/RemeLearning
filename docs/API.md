...
- [8. Kafka — english-service (consumer)](#8-kafka--english-service-consumer)
...
Lấy profile theo `userId`. **Lỗi**: `404` nếu không tồn tại.
...
- **Request** (multipart form):
  ...
  - `language_code`: string, mặc định `"en"`
- **Response**: `TranscriptionResult` (giống endpoint trên, cũng gộp caption hình ảnh nếu
  `VISION_ENABLED=true`)
...
## 8. Kafka — `english-service` (consumer)
...
- **Consume**: `recording.uploaded` — `RecordingUploadedEvent` (`recording_id`, `user_id`, `s3_bucket`,
  `s3_key`, `language_code`) — nay được publish thật bởi `recording-service` (mục 3)
- **Xử lý**: tải audio từ S3 → convert WAV (PyAV) → Whisper transcription + diarization; nếu
  `VISION_ENABLED=true`, còn sample frame từ video gốc (`app/vision/frame_extractor.py`) và gọi Gemini
  vision (`app/vision/gemini_vision_engine.py`) để caption, gộp thành các `Segment` (`speaker ==
  "vision"`) vào `segments` trước khi publish (`app/vision/pipeline.py`)
...
Phần còn thiếu duy nhất trong pipeline: chưa có producer nào cho `learning.gap.analysis.requested` (xem
mục 12 — cần một service bó transcript cùng lịch sử sai sót của learner rồi publish topic này cho
ai-service), và chưa có enforcement JWT ở bất kỳ đâu (`user-service` mới chỉ phát hành token — xem lưu ý
đầu mục 2).
